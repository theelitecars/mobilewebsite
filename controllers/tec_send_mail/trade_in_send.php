...
$car_make = ( isset($inputData['car_make']) ? validateText($inputData['car_make']) : '');
$car_model = ( isset($inputData['car_model']) ? validateText($inputData['car_model']) : '');
$car_condition = ( isset($inputData['car_condition']) ? validateText($inputData['car_condition']) : '');
$car_specification = ( isset($inputData['car_specification']) ? validateText($inputData['car_specification']) : '');
$car_year = ( isset($inputData['car_year']) ? validateNumber($inputData['car_year']) : '');
$car_mileage = ( isset($inputData['car_mileage']) ? validateNumber($inputData['car_mileage']) : '');
$car_body_type = ( isset($inputData['car_body_type']) ? validateText($inputData['car_body_type']) : '');
$car_option = ( isset($inputData['car_option']) ? validateText($inputData['car_option']) : '');
$car_engine = (isset($inputData['car_engine']) ? validateText($inputData['car_engine']) :"");

$selected_car_link = (isset($inputData['selectedCar']) ? $inputData['selectedCar']['link'] :"");
$selected_car_name = (isset($inputData['selectedCar']) ? $inputData['selectedCar']['title']['rendered'] :"");

$token = (isset($inputData['token']) ? $inputData['token'] :"");

$first_name = (isset($inputData['first_name']) ? validateText($inputData['first_name']) :"");
$last_name = (isset($inputData['last_name']) ? validateText($inputData['last_name']) :"");
$email_address = (isset($inputData['email_address']) ? validateEmail($inputData['email_address']) :"");
$mobile_number = (isset($inputData['mobile_number']) ? validateMobileNumber($inputData['mobile_number']) :"");
$appointment_date = (isset($inputData['appointment_date']) ? date('Y-m-d', strtotime(validateText($inputData["appointment_date"]))) :"");
$appointment_time = (isset($inputData['appointment_time']) ? validateText($inputData["appointment_time"]) :"");
$message = (isset($inputData['message']) ? validateText($inputData['message']) : "");
...
if (!$isValid) {
	...
	try {
		...
		$body = 'The Elite Cars - Trade-In with - '. $selected_car_name .'<br/><br/>';
		...
		$body .= '<strong>Vehicle Interested to Trade-in:</strong> ' . $selected_car_link . '<br/>';
		$body .= '<strong>Message:</strong><br/>' . $message . '<br/><br/>';
		
		$body .= 'This message is from https://m.theelitecars.com';

		$text_body = 'The Elite Cars - Trade-In with - '. $selected_car_name .'\n\n';
		...
		$text_body .= 'Vehicle Interested to Trade-in: ' . $selected_car_link . '\n';
